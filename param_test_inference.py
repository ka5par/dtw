...
def plot_scatter(param, value, sm):
    ...
    accuracy_table = pd.read_csv("data/{}_test_acc_table.csv".format(param))
    ...
    plt.savefig('data/plots/test_scatter/{}_{}_{}_param.png'.format(sm, param, value))
    if value == "accuracy":
        plt.figure(figsize=(6, 6))
        ...
        plt.title(sm + " " + param)
        plt.savefig('data/plots/test_scatter/{}_{}_prof_acc.png'.format(sm, param))


def main(param):
    next_returns = pd.read_csv("data/returns/{}.csv".format("^GSPC"))
    b_s_orders = pd.read_csv("data/{}_test.csv".format(param))
    ...
    accuracy_table.to_csv("data/{}_test_acc_table.csv".format(param))


params = ["twed", "lcss"]  # ["twed", "lcss"]
dict_indexes = {"^GSPC": "S&P 500", "^DJI": "Dow Jones Industrial Average", "^GDAXI": "DAX30", "^FCHI": "CAC 40",
                "^N225": "Nikkei 225"}
